...
<!--CHILDRENS TABLE DESIGN - IMPORTING CHILDREN DETAILS FROM THE DATABASE-->
...
/*RECEIVING CHILDREN DETAILS FROM DATABASE*/
    
$tbl_name="tblchildren";
$sql="SELECT * FROM $tbl_name";
$result=mysql_query($sql);

/*DISPLAYING CHILDREN DETAILS*/

echo "<table class=tableNews align=center>";
while($row = mysql_fetch_array($result)){
	echo "<tr>";
  	echo "<td class=NewsHead>"; 
  	echo "<img src=data:image/jpeg;base64," . base64_encode($row['ChildPhoto']) . " width=290 height=290>". "</td>";
  	echo "<td class=ChildData>" . $row['ChildName'] ."<br><p align=center>".$row['ChildAge']." years old</p><input type=submit class=donateBtn 	value=Donate onclick=gotoDonatePage() />". "</td>";
	echo "<td class=NewsData>" . $row['ChildDesc'] . "</td>";
 	echo "</tr>";
}
echo "</table>";
?>
